...
absl::StatusOr<std::unique_ptr<Connection>> Connection::Open() {
    ...
    return std::unique_ptr<Connection>(
        new Connection(conn, screen, key_symbols));
}
...
absl::Status Connection::SendKeystroke(Keystrokes &keystrokes) {
    ...
    for (Keystroke keystroke : keystrokes) {
        ...
        cookies.push_back(xcb_test_fake_input_checked(
            conn, XCB_KEY_PRESS, code, 0, screen->root, 0, 0, 0));
        cookies.push_back(xcb_test_fake_input_checked(
            conn, XCB_KEY_RELEASE, code, 0, screen->root, 0, 0, 0));
    }
